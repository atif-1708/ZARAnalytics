...
export const Inventory: React.FC = () => {
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          headers.forEach((header, index) => {
            ...
            if (header === 'barcode') p.barcode = values[index]; 
  ...
  const valuationStats = useMemo(() => {
    ...
    filteredProducts.forEach(p => {
      ...
      totalRetailValue += stock * (p.salePrice || 0);
    });
           ...
           {activeTab === 'list' ? (
             <>
                <button 
                  ...
                  className="hidden md:flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl font-bold hover:bg-slate-50 transition-all text-xs uppercase tracking-widest"
                >
                  <FileSpreadsheet size={16} /> Bulk CSV
                </button>
                <button onClick={() => { setEditingProduct(null); setFormData({ sku: '', barcode: '', description: '', costPrice: 0, salePrice: 0, currentStock: 0 }); setIsModalOpen(true); }} className="flex items-center justify-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-xl font-bold shadow-lg hover:bg-slate-800 transition-all text-xs uppercase tracking-widest">
        ...
        <div className="p-8 bg-rose-50 border border-rose-100 rounded-[2rem] flex flex-col md:flex-row items-center gap-8 animate-in slide-in-from-top-4 text-left">
           ...
           <div className="flex-1 space-y-2">
              ...
              <p className="text-sm text-rose-600 font-medium leading-relaxed">
                We've detected an issue with your database structure. Please run the repair script.
              </p>
           </div>
           <div className="flex flex-col gap-2">
             ...
             <button onClick={() => setIsSchemaNoticeOpen(false)} className="text-rose-400 text-[10px] font-bold uppercase tracking-widest hover:underline">Dismiss</button>
           </div>
        </div>
      )}
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between h-40">
                 ...
                 <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Stock Count</p>
                    <h3 className="text-3xl font-black text-slate-900">{valuationStats.totalItems.toLocaleString()}</h3>
                 </div>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between h-40">
                 <div className="flex justify-between items-start">
                    <div className="p-3 bg-rose-50 text-rose-600 rounded-2xl"><PieChart size={24}/></div>
                 </div>
                 <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Asset Value (Cost)</p>
                    <h3 className="text-3xl font-black text-rose-600">{formatZAR(valuationStats.totalCostValue)}</h3>
                 </div>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between h-40">
                 <div className="flex justify-between items-start">
                    <div className="p-3 bg-teal-50 text-teal-600 rounded-2xl"><CircleDollarSign size={24}/></div>
                 </div>
                 <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Retail Value (Potential)</p>
                    <h3 className="text-3xl font-black text-teal-600">{formatZAR(valuationStats.totalRetailValue)}</h3>
                 </div>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between h-40">
                 <div className="flex justify-between items-start">
                    ...
                    <span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-widest">
                       {valuationStats.margin.toFixed(1)}% Avg Margin
                    </span>
                 </div>
                 <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Projected Profit</p>
                    <h3 className="text-3xl font-black text-indigo-600">{formatZAR(valuationStats.potentialProfit)}</h3>
                 </div>
              </div>
           </div>
           ...
           <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
              <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                 <h3 className="font-bold text-slate-800">Asset Breakdown</h3>
              </div>
              <div className="overflow-x-auto">
                 <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200">
                       <tr>
                          ...
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Unit Cost</th>
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Total Cost</th>
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Unit Price</th>
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Total Retail</th>
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Est. Profit</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                       {filteredProducts.map(p => {
                          ...
                          return (
                             <tr key={p.id} className="hover:bg-slate-50/50">
                                <td className="px-6 py-4">
                                   <div>
                                      <p className="text-xs font-black text-slate-800">{p.sku}</p>
                                      <p className="text-[10px] text-slate-500 font-medium truncate max-w-[200px]">{p.description}</p>
                                   </div>
                                </td>
                                <td className="px-6 py-4 text-center">
                                   <span className="font-bold text-xs text-slate-700">{stock}</span>
                                </td>
                                <td className="px-6 py-4 text-right text-xs font-medium text-slate-500">{formatZAR(p.costPrice)}</td>
                                <td className="px-6 py-4 text-right text-xs font-bold text-rose-600">{formatZAR(totalCost)}</td>
                                <td className="px-6 py-4 text-right text-xs font-medium text-slate-500">{formatZAR(p.salePrice)}</td>
                                <td className="px-6 py-4 text-right text-xs font-bold text-teal-600">{formatZAR(totalRetail)}</td>
                                <td className="px-6 py-4 text-right text-xs font-bold text-indigo-600">{formatZAR(totalRetail - totalCost)}</td>
                             </tr>
                          );
                       })}
                    </tbody>
